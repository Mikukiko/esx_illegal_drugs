...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.GreenGrocer.coords, true) < 1.50 then
			if not menuOpen2 then
				ESX.ShowHelpNotification(_U('legal_dealer_prompt'))
...
function OpenGreenGrocer()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'item_shop', {
		title    = _U('legal_dealer_title'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		TriggerServerEvent('esx_illegal:sellItem', data.current.name, data.current.value)
	end, function(data, menu)
		menu.close()
		menuOpen2 = false
	end)
...
Citizen.CreateThread(function()
	for k,zone in pairs(Config.Blip) do
