...
AddEventHandler('esx_illegal:pickedUpHydrochloricAcid', function()
	...
	if (xPlayer.getWeight() + xItem.weight) > xPlayer.getMaxWeight() then
		TriggerClientEvent('esx:showNotification', source, _U('hydrochloric_acid_inventoryfull'))
	else
...
AddEventHandler('esx_illegal:pickedUpSodiumHydroxide', function()
	...
	if (xPlayer.getWeight() + xItem.weight) > xPlayer.getMaxWeight() then
		TriggerClientEvent('esx:showNotification', source, _U('sodium_hydroxide_inventoryfull'))
	else
		xPlayer.addInventoryItem(xItem.name, 1)
	end
end)
...
AddEventHandler('esx_illegal:pickedUpSulfuricAcid', function()
	...
	if (xPlayer.getWeight() + xItem.weight) > xPlayer.getMaxWeight() then
		TriggerClientEvent('esx:showNotification', source, _U('sulfuric_acid_inventoryfull'))
	else
		xPlayer.addInventoryItem(xItem.name, 1)
	end
end)
...
AddEventHandler('esx_illegal:processMeth', function()
	if not playersProcessingMeth[source] then
		...
		playersProcessingMeth[_source] = ESX.SetTimeout(Config.Delays.MethProcessing, function()
			...
			if (xPlayer.getWeight() + xmeth.weight) > xPlayer.getMaxWeight() then
